...
export function MatchScreen({ barbershop, onKeepSwiping }: MatchScreenProps) {
  return (
    <motion.div
      ...
      animate={{ opacity: 1 }}
      exit={{ opacity: 0 }}
      className="absolute inset-0 z-50 flex flex-col bg-background/95 backdrop-blur-xl overflow-y-auto"
    >
      <div className="flex min-h-full w-full flex-col items-center px-6 pt-12 pb-12">
        {/* Celebratory header */}
        <motion.div
        ...
        <motion.div
          ...
          className="mt-6 flex-shrink-0 w-full max-w-sm overflow-hidden rounded-[2rem] bg-card border border-white/10 shadow-[0_32px_64px_rgba(0,0,0,0.5)]"
        >
        ...
        <motion.div
          ...
          className="mt-8 flex-shrink-0 flex w-full max-w-sm flex-col gap-4 pb-12"
        >
          <Button
